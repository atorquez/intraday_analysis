...
if ticker:
    ...
    # ---------------------------------------------------------
    # COVERAGE STATUS BADGE (NEW)
    # ---------------------------------------------------------
    def classify_coverage(info, scores):
        """
        Determine coverage level based on presence of key fields.
        FULL: All major fields present
        PARTIAL: Some fields present, but key metrics missing
        NONE: No meaningful fundamentals available
        """

        if info is None or scores is None:
            return "NONE"

        required_fields = [
            scores.get("_raw_forward_pe"),
            scores.get("_raw_trailing_pe"),
            scores.get("_raw_rev_growth"),
            scores.get("_raw_earn_growth"),
            scores.get("_raw_profit_margin"),
            scores.get("_raw_beta"),
            scores.get("_raw_dte"),
            info.get("marketCap"),
            info.get("fullTimeEmployees"),
        ]

        missing = sum(1 for f in required_fields if f in (None, "N/A"))
    ...
    coverage = classify_coverage(info, scores)

    # Coverage badge display
    if coverage == "NONE":
        st.error(
            f"❌ **Fundamentals Coverage: NONE**\n\n"
            f"Yahoo Finance does not provide fundamentals for **{ticker}**.\n"
            "This is normal for mid‑caps, industrials, regional banks, and low‑coverage tickers."
        )
        st.stop()

    elif coverage == "PARTIAL":
        st.warning(
            f"⚠️ **Fundamentals Coverage: PARTIAL**\n\n"
            f"Some metrics for **{ticker}** are missing or incomplete.\n"
            "Displayed values may be limited."
        )

    else:
        st.success(
            f"🟢 **Fundamentals Coverage: FULL**\n\n"
            f"All major fundamental fields are available for **{ticker}**."
        )

    # ---------------------------------------------------------
    # EXECUTIVE SUMMARY + FUNDAMENTALS DISPLAY
    # ---------------------------------------------------------
    summary = {
