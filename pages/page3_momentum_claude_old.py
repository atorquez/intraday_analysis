# ==============================================================================
# 🚀 UNIVERSAL MOMENTUM SCANNER — IMPROVED (FIXED)
#
# DESIGN PRINCIPLE:
...
#
# BUGFIXES (this pass):
#   A. st.cache_data.clear() replaced with fetch_clean_market_batch.clear()
#      — previously a manual "Run" wiped the 6h float/market-cap cache too,
#      forcing a slow, rate-limit-risky re-fetch of .info for every ticker
#      on every scan.
#   B. Observation timeline no longer appends on every Streamlit rerun
#      (e.g. moving a filter slider). It now only appends once per actual
#      scan, guarded by a "new scan just ran" flag + Data_As_Of dedupe.
#   C. float_source is now surfaced in the output table, so a
#      sharesOutstanding_fallback float (much less reliable) is visible.
#   D. fetch_clean_market_batch now tolerates a single-ticker universe,
#      where yfinance returns flat (non-MultiIndex) columns instead of
#      ticker-keyed columns.
#   E. Per-ticker processing errors are now counted and surfaced in the UI
#      (previously only visible via print() / server logs).
#   F. VWAP is labeled as a close*volume proxy, not a true HLC/3 VWAP.
#   G. Daily history and intraday bars are now fetched and cached
#      separately (fetch_daily_batch @ 6h TTL, fetch_intraday_batch @ 2min
#      TTL) instead of one combined function on a shared 2-minute TTL.
#      3mo of daily history doesn't change intraday, so re-downloading it
#      for the whole universe on every 2-minute scan was pure wasted time
#      after the first scan of the session. A manual "Run" now only
#      force-clears the intraday cache, not the daily one.
# ==============================================================================
...
st.caption(
    "Version: V2.1 2026-09-22 — Momentum + Continuation + Actual Price Movement (bugfix pass)"
)
...
# BUGFIX B: track whether a real scan just ran this rerun, and the last
# Data_As_Of / Scan timestamp we've already recorded, so filter-slider
# reruns don't create duplicate timeline entries.
if "momentum_just_scanned" not in st.session_state:
    st.session_state["momentum_just_scanned"] = False

if "momentum_last_recorded_as_of" not in st.session_state:
    st.session_state["momentum_last_recorded_as_of"] = None

# BUGFIX E: track per-ticker error count from the most recent scan.
if "momentum_skipped_count" not in st.session_state:
    st.session_state["momentum_skipped_count"] = 0
...
# DATA FETCH
#
# BUGFIX G: daily and intraday batches now use separate cache TTLs instead
# of one combined fetch on a shared 120s TTL. 3mo of daily history doesn't
# change intraday — it only needs to be refreshed roughly once per trading
# session, not on every 2-minute scan. Under the old combined function, a
# universe of thousands of tickers meant every scan after the first
# re-downloaded that entire daily history for nothing. Splitting these
# means only the first scan of the session pays the daily-fetch cost;
# every scan after that reuses the cached daily batch and only re-fetches
# the 1-minute intraday bars, which are what actually need to stay fresh.
# ==============================================================================
...
def fetch_intraday_batch(tickers_tuple):
...
def momentum_rank_universe_batch(
    ...
    # and the flat-column layout yfinance returns for a single-ticker universe.
    # --------------------------------------------------------------------------
    daily_is_multiindex = isinstance(batch_daily.columns, pd.MultiIndex)
    intra_is_multiindex = isinstance(batch_intra.columns, pd.MultiIndex)

    try:
        if daily_is_multiindex:
            available_daily = set(
                batch_daily.columns.get_level_values(0)
            )
        else:
            # BUGFIX D: a single-ticker download returns flat columns
            # (Open/High/Low/Close/Volume), not ticker-keyed columns. Treat
            # that as "available" only when exactly one ticker was requested,
            # since we can't disambiguate which ticker flat columns belong to.
            available_daily = set(tickers) if len(tickers) == 1 else set()
    except Exception:
        available_daily = set(tickers) if len(tickers) == 1 else set()

    try:
        if intra_is_multiindex:
            available_intra = set(
                batch_intra.columns.get_level_values(0)
            )
        else:
            available_intra = set(tickers) if len(tickers) == 1 else set()
    except Exception:
        available_intra = set(tickers) if len(tickers) == 1 else set()
    ...
    for ticker in active_pool:
        ...
        try:
            # ------------------------------------------------------------------
            # Extract ticker slices. Handle flat-column single-ticker case.
            # ------------------------------------------------------------------
            if daily_is_multiindex:
                daily_df = batch_daily[ticker].copy().dropna(subset=["Close"])
            else:
                daily_df = batch_daily.copy().dropna(subset=["Close"])

            if intra_is_multiindex:
                intraday_df = batch_intra[ticker].copy().dropna(subset=["Close"])
            else:
                intraday_df = batch_intra.copy().dropna(subset=["Close"])
            ...
            # NOTE: this is a close*volume proxy, not a true HLC/3-weighted
            # VWAP. Treat it as an approximation, not a broker-matching VWAP.
            # ------------------------------------------------------------------
...
# BUGFIX B: reset the "just scanned" flag at the top of every rerun. It will
# only be set back to True below if a real scan executes in this run.
st.session_state["momentum_just_scanned"] = False
...
if run_momentum:
    ...
    try:
        # BUGFIX A / G: only invalidate the INTRADAY cache on a manual run,
        # not every @st.cache_data function in the app, and not the daily
        # cache either. Daily history is now cached for 6h (see BUGFIX G
        # above) precisely so a manual "Run" doesn't force a full
        # multi-thousand-ticker daily re-download every time — only the
        # 1-minute bars, which genuinely need to be fresh, get force-
        # refreshed here. The float/market-cap cache (6h) is untouched too.
        ...
        progress_bar.progress(
            30,
            text="Loading daily history..."
        )

        # First scan of the session: full fetch, ~6h cache. Every scan
        # after that within the same 6h window: served from cache, fast.
        daily_fetch_start = time.time()
        raw_daily = fetch_daily_batch(
            tuple(universe_list)
        )
        daily_fetch_seconds = time.time() - daily_fetch_start

        progress_bar.progress(
            55,
            text="Loading intraday data..."
        )

        # Always force-fresh on a manual run (see fetch_intraday_batch.clear()
        # above) since this is the data that actually needs to be current.
        raw_intra = fetch_intraday_batch(
            tuple(universe_list)
        )
        ...
        ranking, skipped_count = momentum_rank_universe_batch(
            universe_list,
            raw_daily,
            raw_intra,
            min_price,
        ...
        st.session_state["momentum_skipped_count"] = skipped_count
        ...
        # BUGFIX B: mark that a real scan just ran, so the timeline-append
        # logic below (which runs on every rerun) knows this rerun is the
        # one that should actually record an observation.
        st.session_state["momentum_just_scanned"] = True
        ...
        st.write(
            ...
            f"(daily fetch: {daily_fetch_seconds:.2f}s)"
        )

        if daily_fetch_seconds < 2.0:
            st.caption(
                "✅ Daily history served from cache (fast) — cached for 6h, "
                "not re-downloaded every scan."
            )

        # BUGFIX E: surface skipped-ticker count in the UI, not just print().
        if skipped_count > 0:
            st.warning(
                f"⚠️ {skipped_count} ticker(s) were skipped due to processing "
                "errors during this scan. See server logs for details."
            )
    ...
    if ranking is not None and not ranking.empty:
        ...
        if filtered.empty:
            ...
            st.caption(
                ...
                "Float_Source shows whether Float is real floatShares data or a "
                "less-reliable sharesOutstanding fallback."
            )
            ...
            #
            # BUGFIX B: this block previously ran (and appended) on every
            # Streamlit rerun, including pure UI interactions like moving a
            # filter slider — which do not correspond to a real 2-minute
            # scan. It now only appends when:
            #   1. a real scan just ran this rerun (momentum_just_scanned), and
            #   2. this scan's Data_As_Of hasn't already been recorded.
            # ==================================================================
            current_as_of = None
            if "Data_As_Of" in display_df.columns:
                as_of_vals = display_df["Data_As_Of"].dropna().unique()
                if len(as_of_vals) > 0:
                    current_as_of = as_of_vals[0]

            should_record = (
                st.session_state["momentum_just_scanned"]
                and current_as_of is not None
                and current_as_of != st.session_state["momentum_last_recorded_as_of"]
            )
            ...
            if st.session_state["momentum_history"]:
                ...
                st.caption(
                    ...
                    "One entry is recorded per completed scan (not per UI interaction)."
                )
