...
def ema_alignment_engine(
    ...
    for ticker in active_pool:
        ...
        r["Price"] = round(current_price, 2)

        if not np.isfinite(current_price):
            r["Reason"] = "Current intraday price invalid"
            rejection_rows.append(r)
            continue
        ...
        # PRICE DROP RULE (NEW)
        # ====================================================
        # Require current price to be at least 5% below previous close
        prev_close = float(daily_df["Close"].iloc[-2])
        drop_pct = (prev_close - current_price) / prev_close

        r["Drop%"] = round(drop_pct * 100, 2)

        if drop_pct < 0.05:
            r["Reason"] = (
                f"Drop {drop_pct*100:.2f}% < required 5% drop"
            )
            rejection_rows.append(r)
            continue
