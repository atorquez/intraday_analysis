...
execution_filter = st.multiselect(
    ...
    default=["Watch List", "Not Watch List", "Crossing Soon", "Setup Only"],
...
def render_results(filtered, ranking, regime, sp500_trend, nasdaq_trend):
    st.metric(
        ...
        value=f"{regime} (SP500: {sp500_trend} | NASDAQ: {nasdaq_trend})"
    )
                ...
                if prime_time_flag:
                    ...
                    if not df_daily.empty and len(df_daily) >= 5:
                        ...
                        if required_cols.issubset(df_daily.columns):
                            ...
                            SRC = (
                                drop_pct >= 0.03
                                and recovery_prob >= 0.60
                                and ema_cross
                                and regime_flag in ["Bearish", "Choppy"]
                            )
