    ...
    for ticker in active_pool:
        try:
            ...
            if abs(intraday_velocity_slope) < 0.10:
                ...
                intraday_velocity_penalty = +2.0      # strong continuation
            elif intraday_velocity_slope < -0.20:
                intraday_velocity_penalty = -2.0      # meaningful downward drift
            else:
            ...
            rows.append({
                ...
                "Velocity_Penalty": intraday_velocity_penalty
            })
        except Exception:
            continue
...
min_price = st.number_input("Minimum Asset Close Gate Price ($)", value=40.0, min_value=40.0, max_value=120.0, key="intraday_min_price")
max_price = st.number_input("Maximum Asset Close Gate Price ($)", value=120.0, min_value=40.0, max_value=120.0, key="intraday_max_price")
